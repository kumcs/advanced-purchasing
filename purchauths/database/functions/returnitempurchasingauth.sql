...
BEGIN
  ...
  -- Check Authorisation by Expense Category
	 ...
	 AND purchauths_maxlevel >= _rec.poitem_value)
       ORDER BY purchauths_expcat_id DESC LIMIT 1;
     IF (NOT FOUND) THEN
       RETURN false;
     END IF;    	 
  ELSE
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION purchauths.returnitempurchasingauth(integer)
  OWNER TO admin;
GRANT ALL ON FUNCTION purchauths.returnitempurchasingauth(integer)
  TO xtrole;
